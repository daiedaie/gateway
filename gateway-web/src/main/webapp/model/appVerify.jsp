...
<script type="text/javascript">
  ...
  <body class="FrameMain">
	...
	<form id="verifyForm" name="verifyForm" action="service/modelDataApp_verifyApp.do">
    ...
    <div id="ruleDiv" >
		...
		<c:if test="${ruleViewList != null && type=='1' }">
			<div style="margin-bottom:10px">
				规则类型：<g:sysDict dictCode="DICT_DESEN_TYPE" dictKey="1"/> <br/>
				...
				<table width="100%" border="0">
						...
						<c:forEach var="o" items="${ruleViewList}" varStatus="status">
							<tr name="fieldTr">
								...
								<td>
									<c:if test="${serviceVO.serviceSource != '2' && groupDict != null && not empty groupDict}">
							...
							<td name="conditionTd<c:if test="${dictByRule eq '1'}">2</c:if>">
								<input readonly="readonly" type="text"
									id="conditionContent${status.index}"
									name="desenFieldList[${status.index}].conditionContent"
									value="${o.conditionContent }" style="width:100px" />
							</td>
						</tr>
					</c:forEach>
				</table>
			...
			<c:if test="${checkRuleList != null && type=='2'}">
				<div style="margin-bottom:10px">
					规则类型：<g:sysDict dictCode="DICT_DESEN_TYPE" dictKey="2"/> <br/>
					不合规最大检查行数：${modelDataFetchVO.maxCheckNum } <br/>
					合规文件输出行数：${modelDataFetchVO.outputNum } <br />
					合法文件是否需要审核：${modelDataFetchVO.checkAudit==1?"是":"否" }
				</div>
		...
		<div id="preRuleDiv">	
			...
			<div class="main_list" style="padding:0 20px 0 20px;display:none" id="preDesenCheckDiv">
				<c:if test="${preDesenType!=null}">
					...
					<c:if test="${preRuleViewList != null }">
						<table width="100%" border="0">
								<tr>
									<th>选择</th>
									<th>字段编码</th>
					...
					<c:if test="${preCheckRuleList != null}">
						<table width="100%" border="0">
							<tr>
								<th>字段编码</th>
								<th>字段名称</th>
