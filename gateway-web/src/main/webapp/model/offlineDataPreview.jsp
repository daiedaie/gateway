...
<script type="text/javascript">
	...
	function search(){
		...
		var chooseDataCount = $("#chooseDataCount").val();
		if(chooseDataCount != '' && isNaN(chooseDataCount)){
			alert("抽样比列只能为数字");
			$("#chooseDataCount").select();
			return false;
		}
		searchPage();
	}
...
<form id="searchForm" action="service/modelTask_offlinePreviewServiceData.do" method="post" loadContainer="pageDataList">
...
<div class="main_search">
    <table width="100%" border="0">
         <tr>
              <td width="80%">
              		抽样类型：
              		<select name="pageObject.orderby">
              			<option value="">从头往后取</option>
              			<option value="desc">从后往前取</option>
              			<option value="random">随机抽取</option>
              		</select>
              		抽样比列：
              		<select name="pageObject.numType">
              			<option value="">条数</option>
              			<option value="%">百分比</option>
              		</select>
              		<input type="text" name="pageObject.chooseDataCount" id="chooseDataCount" value=""/>
              		<input name="" type="button" value="查询" onclick="search(this)"/>
              </td>
         </tr>
    </table>
</div>
<div id="pageDataList">
<div class="main_list">
	脱敏前数据条数：${pageObject.dataCount }，字段数：${fn:length(dataMap['dataFieldMap'])}；
	<c:if test="${fn:length(dataMap['originalDataMap']) > 0}">
	脱敏后数据条数：${dataMap['desenDataCount']}，字段数：${dataMap['desenColumnCount']}
	</c:if>
	<table width="100%" border="0">
		<!-- 表结构元数据 -->
		<tr class="rightSplit">
...
<input type="hidden" name="pageObject.randomPage" id="randomPage" value="${pageObject.randomPage }"/>
...
<div class="main_search">
	<p>
	服务名称：${service.serviceName } &nbsp; 服务编码：${service.serviceCode } &nbsp;&nbsp;
